...
@pytest.mark.anyio
async def test_middleware(test_app1_client: TestClient, test_app1_async_client: AsyncClient):
    init_count = MiddlewareCounter.count

    # global http middleware
    resp = await test_app1_async_client.get('/middleware/global_only')
    assert resp.status_code == 200
    assert MiddlewareCounter.count == init_count+1
    init_count += 1

    # global + local http middleware*2
    resp = await test_app1_async_client.get('/middleware/global_local2')
    assert resp.status_code == 200
