...
2. 写法：
    ...
    - 被`Controller`、`Injectable`、`Service`、`Repository`、`Component`装饰的类的`__init__`方法中声明；`dataclass`中
...
user_router = Inject(APIRouter, 'user接口controller')
app.include_router(user_router)
...
@dataclass
class UserController:
    user_service: Annotated[UserService, 'user_service1']
